import type { Board, Color, EvalResult, Port, Side } from './types';
 ...
 */
export function portTarget(port: Port, board: Board): { cellIndex: number; side: Side } {
...
export function evaluate(board: Board): EvalResult {
  ...
  // 3. portsByRoot を作る
  const portsByRoot = new Map<number, Port[]>();
  ports.forEach((port, i) => {
  ...
  for (let i = 0; i < cellCount; i++) {
    if (cells[i].type !== 'cross') continue;
  ...
  const rootColor = new Map<number, Color>();
  ...
  const junctionColors = new Map<number, Color>();
  for (let i = 0; i < cellCount; i++) {
    ...
    for (let slot = 0; slot < wires.length; slot++) {
      ...
      if (c) junctionColors.set(junctionId(i, slot), c);
  ...
  // 8. 完成チェック
  const colorPorts = new Map<Color, Port[]>();
