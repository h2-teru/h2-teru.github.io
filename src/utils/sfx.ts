...
function shouldPlay(id: SfxId, now: number) {
...
function noise(ctx: AudioContext, start: number, duration: number, gain: number, filterHz: number) {
  if (!masterGain) return;
...
export function playSfx(id: SfxId) {
  const ctx = getAudioContext();
  if (!ctx || !shouldPlay(id, performance.now())) return;
  if (ctx.state === 'suspended') {
    void ctx.resume();
  }
