...
import type { Board, Color, EvalResult, Port, Side } from '../game/types';
...
interface Props {
  ...
  /** 失敗演出 (赤グリッチ) */
  failed?: boolean;
  /** 危険状態 (trace >= 80) — ボーダー色を赤にシフト */
  danger?: boolean;
  /** CLEAR 直後に S から G へ走る接続光 */
  clearFlow?: boolean;
}
...
const CLEAR_FLOW_FLARE: Record<Color, { outer: string; inner: string; core: string }> = {
...
const SIDE_TO_VEC: Record<Side, [number, number]> = {
  0: [0, -1],
  1: [1, 0],
  2: [0, 1],
  3: [-1, 0],
...
const CLEAR_FLOW_EASING = [0.86, 0, 0.12, 1] as const;
...
function cubicBezierAxis(t: number, a1: number, a2: number) {
  const inv = 1 - t;
  return 3 * inv * inv * t * a1 + 3 * inv * t * t * a2 + t * t * t;
}

function cubicBezierAxisDerivative(t: number, a1: number, a2: number) {
  const inv = 1 - t;
  return 3 * inv * inv * a1 + 6 * inv * t * (a2 - a1) + 3 * t * t * (1 - a2);
}

export function PuzzleCanvas({ board, evalResult, onCellTap, failed, danger, clearFlow = false }: Props) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const containerRef = useRef<HTMLDivElement | null>(null);
  const clearFlowStartedAt = useRef<number | null>(null);
  ...
  function draw() {
    ...
    const cellSize = Math.max(
      8,
      Math.min(
        (W - EDGE_PADDING * 2) / (cols + PORT_MARGIN_RATIO * 2),
        (H - EDGE_PADDING * 2) / (rows + PORT_MARGIN_RATIO * 2),
      ),
    );
    ...
    const accentA = (a: number) =>
      danger ? `rgba(255,77,109,${a})` : `rgba(77,163,255,${a})`;
    ...
    for (let r = 0; r < rows; r++) {
      for (let cc = 0; cc < cols; cc++) {
        ctx.beginPath();
        ctx.arc(
          offsetX + cc * cellSize + cellSize / 2,
    ...
    // ── エッジ目盛り線 ────────────────────────────────────────
    {
      ...
      for (let i = 0; i <= cols; i++) {
        ...
        ctx.beginPath(); ctx.moveTo(tx, offsetY - tPad);          ctx.lineTo(tx, offsetY - tPad - tLen);          ctx.stroke();
        ctx.beginPath(); ctx.moveTo(tx, offsetY + boardH + tPad); ctx.lineTo(tx, offsetY + boardH + tPad + tLen); ctx.stroke();
      }
      for (let i = 0; i <= rows; i++) {
        const ty = offsetY + i * cellSize;
        ctx.beginPath(); ctx.moveTo(offsetX - tPad,          ty); ctx.lineTo(offsetX - tPad - tLen,          ty); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(offsetX + boardW + tPad, ty); ctx.lineTo(offsetX + boardW + tPad + tLen, ty); ctx.stroke();
      }
      ctx.restore();
    }
  ...
  function buildClearFlowPaths(
    ...
    if (!evalResult || evalResult.status !== 'CLEAR') return [];

    const graph = new Map<string, FlowEdge[]>();
    const addEdge = (from: string, to: string, points: Point[], color: Color) => {
    ...
    const sideKey = (cellIndex: number, side: Side) => `c:${cellIndex}:${side}`;
    const portKey = (portIndex: number) => `p:${portIndex}`;
    const cellCenter = (cellIndex: number): Point => {
    ...
    const sidePoint = (cellIndex: number, side: Side): Point => {
    ...
    const cellHasSide = (cellIndex: number, side: Side) =>
      rotatedWires(board.cells[cellIndex]).some((wire) => wireHasSide(wire, side));
    ...
    for (let cellIndex = 0; cellIndex < board.cells.length; cellIndex++) {
      ...
      wires.forEach((wire, slot) => {
        ...
        const isStraight = s1 === ((s2 + 2) % 4);
        addEdge(
          sideKey(cellIndex, s1),
          sideKey(cellIndex, s2),
          isStraight ? [p1, p2] : [p1, center, p2],
          color,
        );
      });
    }
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        for (const side of [1, 2] as Side[]) {
          ...
          if (!cellHasSide(cellIndex, side) || !cellHasSide(neighborIndex, opposite)) continue;
          const jid = findJunctionBySide(cellIndex, side);
          const color = typeof jid === 'number' ? evalResult.junctionColors.get(jid) : undefined;
          if (!color) continue;
          addEdge(
            sideKey(cellIndex, side),
            sideKey(neighborIndex, opposite),
            [sidePoint(cellIndex, side), sidePoint(neighborIndex, opposite)],
            color,
          );
        }
      }
    ...
    board.ports.forEach((port, portIndex) => {
      ...
      if (!cellHasSide(target.cellIndex, target.side)) return;
      addEdge(
        portKey(portIndex),
        sideKey(target.cellIndex, target.side),
        [portPos(port, offsetX, offsetY, cellSize, rows, cols), sidePoint(target.cellIndex, target.side)],
        port.color,
      );
    });
    ...
    return colors
      ...
      .filter((path): path is FlowPath => Boolean(path && path.length > 0));

    function findJunctionBySide(cellIndex: number, side: Side): number | null {
  ...
  function findFlowPolyline(
    ...
    const queue = [start];
    const previous = new Map<string, { node: string; edge: FlowEdge } | null>();
    ...
    for (let head = 0; head < queue.length; head++) {
      ...
      for (const edge of graph.get(node) ?? []) {
        if (edge.color !== color || previous.has(edge.to)) continue;
  ...
  function strokePolylinePortion(
    ...
    if (end <= 0 || points.length < 2) return;
  ...
  function portTargetCell(port: Port, rows: number, cols: number): { cellIndex: number; side: Side } {
    switch (port.side) {
      case 0:
  ...
  function portPos(
    ...
    const cellCenter = (i: number) => offsetX + i * cellSize + cellSize / 2;
    const cellCenterY = (i: number) => offsetY + i * cellSize + cellSize / 2;
  ...
  function handlePointer(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    const cellSize = Math.max(
      8,
      Math.min(
        (rect.width - EDGE_PADDING * 2) / (cols + PORT_MARGIN_RATIO * 2),
        (rect.height - EDGE_PADDING * 2) / (rows + PORT_MARGIN_RATIO * 2),
      ),
    );
    const boardW = cellSize * cols;
    const boardH = cellSize * rows;
