...
import type { Board, Color, EvalResult } from '../game/types';
import { evaluate } from '../game/evaluator';
import { generateBoard } from '../game/generator';
import { DEFAULT_TRACE, elevateForDifficulty, type TraceConfig } from '../game/trace';
import { type SkillId, applySkillsToTrace, pickRandomSkills } from '../game/skills';
import { type EquipId, type IntelItem, EQUIPMENT_LIST, generateIntelForStage } from '../game/market';

export type Screen =
  | 'title' | 'home' | 'stage_select' | 'shop' | 'enhancement'
  | 'briefing' | 'skill_pick' | 'hack' | 'wave_result' | 'result';
export type HackOutcome = 'pending' | 'clearing' | 'cleared' | 'traced' | 'stage_cleared';

export const WAVES_PER_STAGE = 10;
const CLEAR_FREEZE_MS = 1000;
const SKILL_PICK_BEFORE_WAVES = new Set([0, 3]);

const PUZZLE_COLORS: Record<number, Color[]> = {
  1: ['red', 'blue'],
  2: ['red', 'blue'],
  3: ['red', 'blue', 'yellow'],
  4: ['red', 'blue', 'yellow'],
...
function buildBoardForWave(stageLevel: number, waveIndex: number, equipment: EquipId[] = []) {
...
interface GameState {
  ...
  beginWave: () => void;
  rotateCell: (cellIndex: number) => void;
  markTraced: () => void;
  consumeFirewall: () => void;
  continueAfterWave: () => void;
  next: () => void;
  debugPlayClearSequence: () => void;

  // Market actions
  sellIntel: (ids: string[]) => void;
  buyEquipment: (id: EquipId) => void;
}
...
export const useGameStore = create<GameState>()(persist((set, get) => ({
  ...
  rotateCell: (cellIndex: number) => {
    const { board, outcome } = get();
    if (!board || outcome !== 'pending') return;
    ...
    newCells[cellIndex] = {
      ...cell,
      rotation: ((cell.rotation + 1) % 4) as 0 | 1 | 2 | 3,
    };
    const newBoard: Board = { ...board, cells: newCells };
    ...
    if (cleared) {
      ...
      setTimeout(() => {
        if (get().outcome !== 'clearing') return;
  ...
  markTraced: () => {
    if (get().outcome !== 'pending') return;
    set({ outcome: 'traced' });
    setTimeout(() => {
      if (get().outcome === 'traced') set({ screen: 'result' });
    }, 1500);
  },
  ...
  buyEquipment: (id: EquipId) => {
    const { coins, purchasedEquipment } = get();
    const item = EQUIPMENT_LIST.find(e => e.id === id);
    if (!item || coins < item.price || purchasedEquipment.includes(id)) return;
    set({
